object FSelPlayer: TFSelPlayer
  Left = 0
  Top = 0
  BorderStyle = bsDialog
  Caption = #1053#1072#1095#1072#1090#1100' '#1080#1075#1088#1091
  ClientHeight = 198
  ClientWidth = 274
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poMainFormCenter
  OnClose = FormClose
  DesignSize = (
    274
    198)
  PixelsPerInch = 96
  TextHeight = 13
  object Label3: TLabel
    Left = 18
    Top = 9
    Width = 76
    Height = 13
    Caption = #1042#1099#1073#1077#1088#1080' '#1080#1075#1088#1086#1082#1072
  end
  object GroupBox1: TGroupBox
    Left = 18
    Top = 60
    Width = 241
    Height = 98
    Anchors = [akLeft, akTop, akRight]
    Caption = '                        '
    TabOrder = 2
    object Label1: TLabel
      Left = 36
      Top = 69
      Width = 63
      Height = 13
      Caption = #1048#1084#1103' '#1089#1077#1088#1074#1077#1088#1072
    end
    object rbnServer: TRadioButton
      Left = 20
      Top = 22
      Width = 109
      Height = 17
      Caption = #1057#1086#1079#1076#1072#1090#1100' '#1089#1077#1088#1074#1077#1088
      Checked = True
      TabOrder = 0
      TabStop = True
      OnClick = rbnClientClick
    end
    object rbnClient: TRadioButton
      Left = 20
      Top = 46
      Width = 109
      Height = 17
      Caption = #1055#1088#1080#1089#1086#1077#1076#1080#1085#1080#1090#1100#1089#1103
      TabOrder = 1
      OnClick = rbnClientClick
    end
    object edServerIp: TEdit
      Left = 114
      Top = 66
      Width = 117
      Height = 21
      Hint = 'IP '#1072#1076#1088#1077#1089' '#1080#1083#1080' '#1089#1077#1090#1077#1074#1086#1077' '#1080#1084#1103' '#1082#1086#1084#1087#1100#1102#1090#1077#1088#1072
      ParentShowHint = False
      ShowHint = True
      TabOrder = 2
      Text = '192.168.25.3'
    end
  end
  object cbPlayer: TDBLookupComboboxEh
    Left = 18
    Top = 25
    Width = 241
    Height = 21
    Anchors = [akLeft, akTop, akRight]
    DropDownBox.Columns = <
      item
        FieldName = 'NAME'
        Title.Caption = #1048#1075#1088#1086#1082
        Width = 100
      end
      item
        FieldName = 'WINNED'
        Title.Caption = #1042#1099#1080#1075#1088
        Width = 40
      end
      item
        FieldName = 'FAILED'
        Title.Caption = #1055#1088#1086#1080#1075
        Width = 40
      end
      item
        FieldName = 'SCORES'
        Title.Caption = #1042#1099#1080#1075#1088#1099#1096
        Width = 50
      end>
    DropDownBox.ShowTitles = True
    DropDownBox.Width = 260
    EmptyDataInfo.Text = '< '#1053#1077#1090' >'
    EditButtons = <>
    KeyField = 'ID'
    ListField = 'NAME'
    ListSource = dsoHumans
    TabOrder = 0
    Visible = True
  end
  object btnStart: TBitBtn
    Left = 52
    Top = 165
    Width = 83
    Height = 27
    Anchors = [akLeft, akBottom]
    Caption = #1054#1082
    Default = True
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 3
    OnClick = btnStartClick
    Glyph.Data = {
      
      FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
  end
  object btnCancel: TBitBtn
    Left = 141
    Top = 165
    Width = 83
    Height = 27
    Anchors = [akLeft, akBottom]
    Cancel = True
    Caption = #1054#1090#1084#1077#1085#1072
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 4
    OnClick = btnCancelClick
    Glyph.Data = {
      
      FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
  end
  object chbNetwork: TCheckBox
    Left = 30
    Top = 57
    Width = 97
    Height = 17
    Caption = #1048#1075#1088#1072#1090#1100' '#1087#1086' '#1089#1077#1090#1080
    TabOrder = 1
    OnClick = chbNetworkClick
  end
  object mtHumans: TMemTableEh
    Params = <>
    Left = 144
    Top = 24
    object mtHumansID: TStringField
      FieldName = 'ID'
      Size = 16
    end
    object mtHumansNAME: TStringField
      FieldName = 'NAME'
      Size = 255
    end
    object mtHumansALL: TIntegerField
      FieldName = 'ALL'
    end
    object mtHumansCOMPLETED: TIntegerField
      FieldName = 'COMPLETED'
    end
    object mtHumansINTERRUPTED: TIntegerField
      FieldName = 'INTERRUPTED'
    end
    object mtHumansWINNED: TIntegerField
      FieldName = 'WINNED'
    end
    object mtHumansFAILED: TIntegerField
      FieldName = 'FAILED'
    end
    object mtHumansSCORES: TIntegerField
      FieldName = 'SCORES'
    end
    object mtHumansTOTAL: TIntegerField
      FieldName = 'TOTAL'
    end
    object mtHumansPASSWORD: TStringField
      FieldName = 'PASSWORD'
      Visible = False
      Size = 255
    end
  end
  object dsoHumans: TDataSource
    DataSet = mtHumans
    Left = 176
    Top = 24
  end
end
